...
const PatientTreatment = ({ userid, onclose }) => {
  ...
  //       console.log("All treatments:", response.data);
  //     } else {
  //       console.error("Fout bij het ophalen van alle behandelingen:", response.message);
  ...
  const updateTreatment = async () => {
    try {
      const response = await postCall("updateTreatment", { id: selectedTreatmentid, userid: userid, treatment: treatment, note: note });
      if (response.isSuccess) {
        setRecenteBehandelingen((prev) => prev.map((item) => item.id === selectedTreatmentid ? { ...item, treatment: treatment, note: note } : item));
  ...
  const deleteTreatment = async () => {
    try {
      const response = await postCall("deleteTreatment", { id: selectedTreatmentid, userid: userid });
      if (response.isSuccess) {
        setRecenteBehandelingen((prev) => prev.filter((item) => item.id !== selectedTreatmentid));
        setSelectedTreatmentid(null);
        setTreatment('');
        setNote('');
        openToast(response.message);
      } else {
        openToast("Fout bij het verwijderen van behandeling:", response.message);
      }
    } catch (error) {
      openToast("Er is een fout opgetreden:", error);
    }
          ...
          <div className="recente-behandelingen-box">
            ...
            <div className="behandelingen-grid">
              {recenteBehandelingen.map(behandeling => (
                <div key={behandeling.id} className="behandeling-item-patient" onClick={() => {
          ...
          <div className="form-section-treatments">
            ...
            {selectedTreatmentid === null ? (
              ...
              <div className='button-group'>
                <button className="btn-secondary" onClick={() => deleteTreatment()}>
                  Verwijderen
                </button>
                <button className="btn-primary" onClick={() => updateTreatment()}>
